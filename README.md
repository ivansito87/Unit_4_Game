...
3. Here's how the app works:
   ...
   * When the player clicks on a crystal, it will add a specific amount of points to the player's total userSum. 

     * Your game will hide this amount until the player clicks a crystal.
     * When they do click one, update the player's userSum counter.

   * The player wins if their total userSum matches the random number from the beginning of the game.

   * The player loses if their userSum goes above the random number.

   * The game restarts whenever the player wins or loses.

     * When the game begins again, the player should see a new random number. Also, all the crystals will have four new hidden values. Of course, the user's userSum (and userSum counter) will reset to zero.
